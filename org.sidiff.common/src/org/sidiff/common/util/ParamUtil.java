...
public class ParamUtil {
	 ...
	 */
	private List<Argument<String>> arguments = new LinkedList<>();

	/**
	 * Map of defined parameters. Maps from the parameter name to its object.
	 */
	private Map<String, Argument<?>> parameterMap = new HashMap<>();

	/**
	 * Map of defined flags. Maps from the name of the flag to its object.
	 */
	private Map<String, Argument<Boolean>> flagMap = new HashMap<>();
	...
	public Argument<String> createArgument(boolean required) {
		...
		Argument<String> result = new Argument<>(true, String.class);
	...
	public Argument<Boolean> createFlag(String key) {
		...
		Argument<Boolean> result = new Argument<>(true, Boolean.class);
	...
	public <T> Argument<T> createParameter(String key, Class<T> type, boolean required) {
		...
		Argument<T> result = new Argument<>(required, type);
		...
		private void setValue(String value) {
			if (Collection.class.isAssignableFrom(type)){
				...
				for (String value2 : values) {
					try {
						((Collection<? super String>)this.value).add(value2);
					} catch (Exception e) {
